...
Completed work moves from TODO to decisions. It is not left checked-off in TODO. State (Active/Deferred) is orthogonal to priority: an item can be important yet Deferred because it is blocked. A Deferred item with no concrete trigger is not deferred — it is either Active or a "won't do". A "won't do" is recorded in decisions (the dated call) AND kept as a one-line entry in a frozen "won't-do" index at the foot of TODO — carrying no rationale (that lives in decisions) and never promoted back to Active/Deferred — so the same item is not re-proposed.
