#!/usr/bin/env python3
"""Minimal dependency-free Chrome DevTools Protocol client for chat.qwen.ai.

Shares ONE dedicated Chrome instance with the gemini-web / chatgpt-web skills
(same debug port and --user-data-dir), so the AI surfaces live as sibling tabs in
a single signed-in browser. Plain Chrome exposes a real debug port — attach over
the WS and drive the renderer directly.
"""
...
def qwen_target(port=PORT):
    """Return the CDP page target for the Qwen tab, or None if absent."""
    for t in http_json("/json/list", port):
        if t.get("type") == "page" and "chat.qwen.ai" in (t.get("url") or ""):
            return t
    return None
...
if __name__ == "__main__":
    print("port alive:", port_alive(), "| qwen target:", bool(qwen_target()))
