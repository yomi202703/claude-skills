...
| `coverage-recheck <slug> [--source <md>] [--judge-model M]` | Measure-only coverage re-run over a committed tree (alias: `coverage-narrative`). Judges the cached QA set against the narrative and writes the gap report; never edits the tree, runs no remediation loop. The iterating form runs automatically inside `narrative-draft` — call this standalone to re-measure after a judge-model outage left coverage `unavailable` (`coverage_pct=null`, not 0%), or against a revised source. Judging is batched (20 QA/call) to stay inside the judge subprocess's timeout. `--source` only needed if no QA set is cached yet. |
...
The other drills train material already acquired. Discovery is how it gets acquired in the first place, and is the main axis of tree learning. Reading a finished tree and nodding is low-density — the learner gets the solution without feeling the problem. Instead: hold the tree as a sealed answer key, hand the learner the problem (a DAG edge = a `⟳ だから次の問題`), and make them reconstruct the resolution before revealing it. Core invariants: never reveal before the learner commits ("分からない" counts); on a divergent answer explain only why it diverges, never the correct answer — they close the last gap themselves; an answer that commits to nothing (an omnibus word — 複雑/バランス/中立/均衡点 — or a procedure word — 流れで/手順通り) is a third state, not a divergence: halt it and force one concrete scene rather than refuting it, unless it is a genuine breakthrough piercing the abstraction. The subject map `maps/<subject>-dag.json` is the question generator (edge `kind` fixes the question type; confirm parents first per `ai_usage`). When the user wants to learn / study / "勉強する" a tree or walk a DAG, first read `reference/discovery-drill.md` and run that loop. Grounded in epistemological obstacles, the genetic method, and productive failure; never scores the learner (hard rule #2) — a divergence is the trigger to capture a misconception card and re-aim, never a verdict.
