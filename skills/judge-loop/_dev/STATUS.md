...
## このセッション(2026-06-27・基準改訂ビュー)
- オーナーの「要件定義の plan を tree、その後のずれを変遷 DAG で」提案を外部2AI(chatgpt-web/gemini-web)へ非相関相談。両者「条件付き採用」一致=DAG は手で育てず decisions から生成・「因果」でなく改訂履歴・ノードは atomic clause。
- 接地で判明: 真実の半分は既存(decisions＋P4 atomic criteria ledger＋run-ledger)・committed/pending も G10＋provenance tier で担保済。「凍結基準tree」は P0u(単位創発)と衝突。
- オーナー裁定=案A: tree=plan 用(単位非依存 scaffold のみ凍結)・DAG=その後(基準が育つ生成 frontier ビュー)。基準の凍結tree は足さない。
- 反映: SKILL.md P4(frontier-view レンダ句)・Composition(review-server render 句)・Ledger(単一真実→生成ビュー＋committed/pending)。review-server の正式 S 項化は別スキル台帳マターゆえ deferred。詳細 decisions/横断.md 2026-06-27。

## このセッション(2026-06-27・repo-shape 新設)
- 6/26 設計確定のディレクトリ再構成スキルを `repo-shape` として骨格 author(repo 非依存の規律のみ・正準名は凍結せず)。検証と §3.5 手術は実リポ到着まで Deferred のまま。
- 「最適ディレクトリ構造」を外部2AI(chatgpt-web/gemini-web)へ非相関相談 → judge-loop テーゼ(no-GT では基準+事例+評価層が真の成果物)が独立裏書き。取り込み=gen/source firewall を三分化(append-only 証拠を追加)・縦割り vs 横割りは検証論点として記録。skill-shape で repo-shape の本文を de-bloat。詳細 decisions/横断.md 2026-06-27。

## このセッション(2026-06-28・別ドメイン到来予告)
- オーナーから事実: 別ドメインが3つ実在。別PCにあり未取得(後で引っ張る)。これが本スキルの Deferred 一斉解除トリガ —— 別ドメイン overfit 検出 / P3 ハーネス+run-ledger / アクセス層スキル(唯一の prerequisite 穴)。
- 到来時の第一手(設計、agentic-engineering 側 grill で確定): (a) アクセス層を立て源泉を読めるようにする → (b) judge-loop を直さず素のまま当て、合わないゲート/フェーズを全部ログる = overfit 地図。先回りで調整しない(街灯バイアス回避)。
- 関連: 別リポ agentic-engineering の Kun 研究が「あかつきは judge-loop の出自=循環ゆえ eval は第二ドメインで初めて非循環」と確認。技術移植候補(programbench-bench の統計層を Gemma flutter/再現性測定へ)を decisions/2026-06-28_s4-shipping-vessel.md に記録。本 STATUS は snapshot 記録のみ、TODO の Deferred→Active 化は実取得時にオーナー判断。

## 次の一手
- handoff ストリームは2本ともクローズ。磨き項目は TODO.md 参照。大半は別ドメイン着手まで Deferred。
- repo-shape は骨格済・残=実リポでの最小版検証→§3.5 手術(TODO Deferred)。
- 基準改訂ビューは review-server に W7(rubric-frontier レンズ)として文書追加済み(新ゲートでなく生成レンズ・S1/S2/S8 下)。template への実装は必要時まで据え置き。詳細 review-server/_dev/decisions.md 2026-06-27。
- TODO Active「/plan 組込み」とは living-artifact 面で交差(当該項は未クローズ)。
