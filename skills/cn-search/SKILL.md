---
name: cn-search
description: 中国一次ソースの取得レイヤー（単一所有）。CN の AI/モデル/市場/規制トピックで「どの中文火元を・どう壁越えして・どんな ZH クエリで」取りに行くかだけを所有する。解釈の目でも裏取りの epistemics でもない——zeitgeist（複数の目）と deep-strict（claim 裏取り）がこの層を compose する。所有物＝CN火元カタログ（reference/sources.md）＋到達手段（WebSearch allowed_domains の cn 寄せ・Gemini 橋・直 WebFetch）＋ZHクエリ作法（中文語彙ブートストラップ・鮮度/canonical 軸）＋CN一次トピック判定の固有名リスト。velocity 計算・複数の目・claim 独立性カウントは所有しない（親が持つ）。Triggers — deep-strict/zeitgeist が CN 一次トピックに当たった時、または「中国の源から取りたい」「CN火元」「中国系を取得」。
---

呼び出し側（zeitgeist の目、deep-strict の裏取り）に CN 火元の「どこを・どう取るか」だけを供給する。取った素材をどう読むか・裏取りするかには立ち入らない。
...
この環境の WebSearch は US 寄りで CN に届きにくい。三段で取りに行く。

- WebSearch の `allowed_domains` を cn ドメインに寄せる（zhihu.com / 36kr.com / juejin.cn / jiqizhixin.com / mp.weixin.qq.com 等。火元の対象ドメインは reference/sources.md）。
- velocity のある CN 火元・動画系は Antigravity(Gemini) 橋で取る（WebSearch と並列）。
...
- 上の二つで届かなければ官方ブログ・GitHub・HuggingFace モデルカードを直 WebFetch で取りに行く。
