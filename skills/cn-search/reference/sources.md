...
| 个人公众号（高信号・発見ヒント） | mp.weixin.qq.com | probe | 宝玉xp(=宝玉的分享, X@dotey) / 归藏的AI工具箱(X@op7418) / AI工程化 / 凌十一 / LangGPT(云中江树) / AGI课堂。qwen-web の種出し由来。名の信頼は実到達で確認する（旧「2モデル一致＝高信頼」は廃止）。直 fetch は不安定＝著者名で WebSearch、知乎/CSDN ミラー経由で本文を拾う |
...
- 小宇宙 播客: OnBoard! / 张小珺的科技产品杂谈 / 十字路口Crossing / 硅谷101。shownotes に群の二維碼
...
qwen-web（CN ネイティブ・非相関）に「定番外で高水準 AI 从业者が集まる圏子」を吐かせる→機械到達するものは上の表へ実測昇格、参加制は人手枠へ。名の信頼は実到達で確認する（旧「2モデル一致」運用は廃止）。Western モデルが出さない辺境を埋める用途。glm-web は CN ネイティブの種出しから外した——2026-06 検証で CN 文脈固有語（赛博对账 等）を字面から誤再構成し qwen のみ正解。GLM はコーディング専用（glm-web/SKILL.md 参照）。
