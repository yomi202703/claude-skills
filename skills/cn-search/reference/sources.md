...
| 个人公众号（高信号・発見ヒント） | mp.weixin.qq.com | probe | 宝玉xp(=宝玉的分享, X@dotey) / 归藏的AI工具箱(X@op7418) / AI工程化 / 凌十一 / LangGPT(云中江树) / AGI课堂。名の信頼は実到達で確認する（旧「2モデル一致＝高信頼」は廃止）。直 fetch は不安定＝著者名で WebSearch、知乎/CSDN ミラー経由で本文を拾う |
...
「定番外で高水準 AI 从业者が集まる圏子」の候補を上の个人公众号・X アカウント・各コミュニティから拾い、機械到達するものは上の表へ実測昇格、参加制は人手枠へ。名の信頼は実到達で確認する（旧「2モデル一致」運用は廃止）。
