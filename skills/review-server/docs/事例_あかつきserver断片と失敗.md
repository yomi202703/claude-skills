...
訂正(2026-06-24・decisions/横断 参照)。当初ここは「2ロールを1サーバーに畳む」を理想とした。
だが1章の棚卸しが示すのは別の事実: 乱立したのは開発者側の検証アングル9断片(横断/keydiff/flutter/checklist/形式/乗換/禁止)であって、
人間GT作成側は factcheck/server.py ただ1つ。anti-sprawl(S1/F1)の根拠は人間面に存在しない。
ゆえに正しい理想形は「1サーバー2ロール」ではなく:

- 開発者側 = 1つの config 駆動 診断サーバー。今ある開発者9断片は、この1サーバーの「レンズ/モード」に畳む(S1 はここに効く)。
- 人間側 = factcheck/server.py の到達点(追記オンリー・snapshot・配布・反対時独立ラベル)を、ドメイン非依存・単位可変・blank-slate/commit後開示 に昇格させた、引き渡し可能な別成果物(別フォルダ/パッケージ)。畳む必要はない=元から1つ。
- 両者を繋ぐのは1プロセスでなく、契約単一ソース(S2)と GT 戻し1経路(S9)。分離はむしろ引き渡し衛生と防火壁(S3=不在による隠蔽)を強くする。

注意(最重要・F2): 最も高くついた失敗はまさにこの分離した人間サーバー(factcheck)で起きた=契約を別に焼き直してドリフト。
教訓は「分けるな」でなく「契約を複写するな」。別成果物の契約は単一ソースから生成し手書きしない。
分離可否(モードか別成果物か)は grill hook。ゲート S2/S3/S6/S9 はどちらでも不変。
