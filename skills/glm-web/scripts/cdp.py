#!/usr/bin/env python3
"""Minimal dependency-free Chrome DevTools Protocol client for chat.z.ai (GLM).

Shares ONE dedicated Chrome instance with the gemini-web / chatgpt-web / qwen-web
skills (same debug port and --user-data-dir), so the AI surfaces live as sibling
tabs in a single signed-in browser. Plain Chrome exposes a real debug port —
attach over the WS and drive the renderer directly.
"""
...
def glm_target(port=PORT):
    """Return the CDP page target for the GLM tab, or None if absent."""
...
if __name__ == "__main__":
    print("port alive:", port_alive(), "| glm target:", bool(glm_target()))
