...
The converter copies structure verbatim — no header-guessing, no splitting choice cells, no collapsing merges; the consuming LLM interprets it.
...
1. Convert (one call = triage + faithful write + self-verify):
   ...
   ```
   Prints a per-sheet manifest, one `.html` per HTML-path sheet. Fields: `path` (`html`|`sqlite`, by `used_cells > 20000`), `faithful` (must be `N/N`, else non-zero exit = a defect to fix), `has_drawings`, `suggests_image`.

2. Act per sheet from the manifest — don't re-decide what it settled:
   - `html` → the `.html` is the artifact; read it to answer/present. Headers, merged-label groups (rowspan/colspan), and transposed layouts reconstruct straight from it.
   - `sqlite` → too large for context. Peek at the top rows, then materialize — pass `--header-rows N` only if the header isn't row 1 (spec block above the table):
     ...
     ```
   - `suggests_image` → drawings present; their text is already in the HTML as `〔図形: …〕`. Render a PNG (`xlsx_visual.py`, see `docs/p6_visual.md`) only when meaning depends on layout the annotations can't convey (arrows, overlap, conditional-format color) — the one judgment left to you. Expensive; one sheet at a time.
