#!/usr/bin/env python3
"""Minimal dependency-free Chrome DevTools Protocol client for gemini.google.com.

Unlike Atlas (which blocks CDP, forcing AppleScript), plain Chrome exposes a
real remote-debugging port, so we attach over the CDP WebSocket and drive the
renderer with Runtime/Input commands — no AppleScript string-escaping, no
Trusted-Types execCommand dance beyond what the page itself requires.

The skill runs a DEDICATED Chrome instance (its own --user-data-dir) so it never
collides with the user's normal Chrome and the Google login persists across runs.
"""
import json, os, socket, base64, struct, urllib.request

PORT = int(os.environ.get("GEMINI_CDP_PORT", "9333"))
...
def gemini_target(port=PORT):
    """Return the CDP page target for the Gemini tab, or None if absent."""
    for t in http_json("/json/list", port):
        if t.get("type") == "page" and GEMINI_URL in (t.get("url") or ""):
            return t
    return None
...
if __name__ == "__main__":
    print("port alive:", port_alive(), "| gemini target:", bool(gemini_target()))
