...
発表 1〜4 ヶ月の preprint 中心トピックでは OpenAlex は空振りするので補助に留め、WebSearch/arxiv を主にする（citation graph が効くのは索引済みの枯れた文献のみ）。
...
言語跨ぎの独立性: ZH 一次 + EN 一次 は独立に数えてよい（組織・著者・言語コミュニティが違う＝high claim の 3 独立を満たしやすくなる実利あり）。ただし EN 告知を翻訳/転載しただけの中文記事は独立ではない（上流が同一）。翻訳・転載は元ソースに畳んで 1 source 扱い。
...
- ZH 一次（中国一次トピックのみ）: ZH 一次が最低 1 本あるか。無ければ西側二次だけで黙って答えず、理由（login-walled / fetch 失敗 / 0 件）を Retry Queue に明示する
...
|----|-------|--------|------------|---------|
| C1 | <atomic claim> | high | confirmed | [URL1 (2026-04, primary), URL2 (2026-03, primary, [zh]), URL3] |
