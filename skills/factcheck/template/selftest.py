...
# The dev server whose store/ingest this GT flows back into. factcheck COMPOSES
# review-server (it does not re-own S2/S6/S9); the S9 round-trip below imports the
# real review-server store to prove the GT lands on the one ingestion path. If
# review-server is not a sibling skill, the round-trip is skipped (not failed).
REVIEW_SERVER_TEMPLATE = os.path.normpath(os.path.join(HERE, "..", "..", "review-server", "template"))
...
def step_roundtrip(csv_text):
    ...
    # factcheck composes review-server; if it is not present, skip (not fail) —
    # the column-contract assertion in step 4 still holds the S9 shape.
    store_py = os.path.join(REVIEW_SERVER_TEMPLATE, "store.py")
    if not os.path.exists(store_py):
        print("5. S9 round-trip .. SKIP (review-server not a sibling; export columns verified in step 4)")
        return
    sys.path.insert(0, REVIEW_SERVER_TEMPLATE)
    from store import Store

    src = json.load(open(os.path.join(REVIEW_SERVER_TEMPLATE, "contract.example.json"), encoding="utf-8"))
